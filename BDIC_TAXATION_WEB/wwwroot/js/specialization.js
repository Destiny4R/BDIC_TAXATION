...
$(document).on('click', '.delete-btn', function () {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: '/v1/GetSpecializationToDelete/' + id,
...
$(document).on('click', '.edit-btn', function () {
    ...
    $.ajax({
        type: "POST",
        url: '/v1/GetSpecializationForEdit/' + id,
        success: function (data) {
            $.unblockUI();
            if (data.success) {
                $('#specialnameid').val(data.data.id); 
                $('#specialnameedit').val(data.data.name);
                $('#specialcode').val(data.data.code);
                const edit = new bootstrap.Modal('#editspecialisationModal')
                edit.show();
